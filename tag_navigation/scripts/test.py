...
class AprilTagFollower:
    def __init__(self):
        ...
        self.desired_distance = rospy.get_param('~desired_distance', 0.1)  # 期望距离（米）
        ...
        self.camera_frame_id = rospy.get_param('~camera_frame_id', 'realsense_up_color_optical_frame')  # 相机坐标系

        # 发布器 - 发布目标位置到move_base
        self.goal_pub = rospy.Publisher('/navigation/move_base_simple/goal', PoseStamped, queue_size=10)

        # 订阅器 - 订阅AprilTag检测结果
        rospy.Subscriber('/tag_detections', AprilTagDetectionArray, self.tag_detection_callback)

        # rospy.Subscriber('/navigation/move_base_simple/goal', PoseStamped, self.goal_msg_callback) 
    ...
    def goal_msg_callback(self,msg):
    ...
    def control_callback(self, event):
        ...
        try:
            ...
            # 获取标签的方向（欧拉角）
            orientation = tag_pose_map.orientation
            ...
            goal_pose.pose.position.x = cam_pose_map.position.x - self.desired_distance * math.cos(adjusted_yaw)
            goal_pose.pose.position.y = tag_pose_map.position.y - self.desired_distance * math.sin(adjusted_yaw)
            goal_pose.pose.position.z = 0.0
            # print(goal_pose.pose.position)
            # print(adjusted_yaw)
            print(cam_pose_map.position)

            # 设置朝向 - 面向标签
            angle_to_tag = math.atan2(
                tag_pose_map.position.y - goal_pose.pose.position.y,
                tag_pose_map.position.x - goal_pose.pose.position.x
            )
